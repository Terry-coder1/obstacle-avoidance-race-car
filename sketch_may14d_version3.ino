...
{
  float total_repel_x = 0, total_repel_y = 0;
  float max_danger_factor = 0; // 用于衡量整体危险程度
  ...
  for (int angle = 0; angle < 360; angle++)
  {
    ...
    if (abs(rel_angle) <= 90)
    {
      ...
      // --- 改进 A: 平滑阈值逻辑 ---
      // 阈值从正前方的 FRONT_THRESHOLD (0.8m) 平滑过渡到侧向的 SIDE_THRESHOLD (0.4m)
      float cos_factor = cos(rad); 
      float dynamic_threshold = SIDE_THRESHOLD + (FRONT_THRESHOLD - SIDE_THRESHOLD) * cos_factor;

      if (dist < dynamic_threshold)
      {
        // --- 改进 B: 指数级权重 (距离越近，力越大) ---
        // 使用平方反比或线性增强，避免远距离噪点干扰
        float weight = (dynamic_threshold - dist) / dynamic_threshold;
        float force_mag = weight * weight * 2.0f; // 平方增强感度

        // 累加排斥力 (反向)
        total_repel_x += force_mag * cos(rad + M_PI);
        total_repel_y += force_mag * sin(rad + M_PI);

        // 记录最大危险系数，用于后面降速
        if (weight > max_danger_factor) max_danger_factor = weight;
      }
    }
  }

  // --- 改进 C: 稳定的目标向量合成 ---
  // 给定一个恒定的前向拉力 (Drive Force)
  float drive_weight = 1.2f; 
  float target_x = drive_weight + total_repel_x;
  float target_y = total_repel_y * 1.5f; // 适当放大横向感度

  // 计算目标角
  float target_rad = atan2(target_y, target_x);
  float target_deg = degrees(target_rad);

  // --- 改进 D: 动态映射转向 ---
  // 增加增益系数，使转向更果断，同时限制在舵机物理范围内
  float steering_gain = 1.2f; 
  int angle_offset = (int)(target_deg * steering_gain);
  out_servo_angle = STEERING_CENTER - constrain(angle_offset, -30, 30);

  // --- 改进 E: 综合减速逻辑 ---
  // 速度不只看正前方 15 度，只要“危险系数”高，或者转向角度大，都要减速
  float speed_factor = 1.0f - constrain(max_danger_factor, 0, 0.8f);
  
  // 如果转向角度很大，也要额外减速（防止侧翻）
  float turn_slow_factor = 1.0f - (abs(angle_offset) / 60.0f); 
  
  float final_speed = MAX_SPEED * speed_factor * turn_slow_factor;

  // 确保基本的蠕动速度，或在极近距离停车
  if (max_danger_factor > 0.9f) { // 距离阈值非常近了 (约 < 10cm)
    out_speed = 0; 
  } else {
    out_speed = (int)constrain(final_speed, MIN_SPEED, MAX_SPEED);
  }
}
